...
func (p *ExampleFrameProcessor) ProcessFrame(frame frames.Frame, direction processors.FrameDirection) {
	...
	if audioFrame, ok := frame.(*frames.AudioRawFrame); ok {
		...
		p.audio = append(p.audio, audioFrame.Audio...)
	}
}

func (p *ExampleFrameProcessor) GetReceivedFrames() []byte {
	return p.audio
}

func TestAudioResponseAggregator(t *testing.T) {
	...
	// 创建音频响应聚合器
	aggregator := NewAudioResponseAggregatorWithAccumulate(
	...
	assert.Equal(t, []byte("Hello World! This is a test."), downstreamProcessor.GetReceivedFrames())
